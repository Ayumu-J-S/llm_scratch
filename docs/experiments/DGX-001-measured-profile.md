...
| 39 | Protocol repair 10 and focused validation | implemented | Scale every candidate by any selected-shape compute slowdown measured in the online pilot; retain oversized immutable cache footprints; persist and gate an initial telemetry sample before trainer/data construction; gate allocated and reserved allocator growth; bind matrix, decomposition, and pilot to one GPU UUID plus driver/CUDA/Torch/OS-kernel identity | Expanded DGX/config/W&B suite: 184 passed in 62.18 seconds; direct repair regressions: 20 passed; no GPU or online W&B work ran |
| 40 | Full CPU validation | passed | Network-free lint, full suite, Hydra preflight, lock drift, disabled/offline W&B smoke, exact 27-arm plan composition, compileall, diff check, and runtime-spec identity all pass after the tenth repair cycle | `make ci-cpu`: 552 passed, 1 skipped in 132.31 seconds; about 425 GB free; no GPU or online W&B work ran |
| 41 | Supplemental exact-head audit | `FAIL` at `5725f5c` | A mutable PASS summary could relabel matrix hardware/software before auxiliary authority was created; impossible negative allocator evidence passed the stability gate; `unchanged: true` overrode differing cache before/after identities | Three CPU adversarial reproductions and PR #47 comment; explicitly not a substitute for formal `/review` |
| 42 | Protocol repair 11 | implemented; re-review pending | Re-derive matrix candidate statistics, selection, and hardware/software identity from all 27 physical runs both before auxiliary plan creation and during auxiliary summarization; require nonnegative integer allocator observations; require exact cache before/after identity | Forged-summary, invalid-allocator-schema, inconsistent-cache, and runner-call regressions |
| 43 | DGX-focused CPU validation | passed | Hardware, planning, runner, runtime-image, telemetry, and all-role watchdog behavior pass together after the supplemental repair | 117 passed in 57.82 seconds; no GPU or online W&B work ran |
| 44 | Expanded focused validation | passed | DGX, resolved-profile/config, and W&B producer/parser behavior pass together; focused Ruff is clean | 197 passed in 73.73 seconds; no GPU or online W&B work ran |
| 45 | Full CPU validation | passed | Network-free lint, full suite, Hydra config preflight, lock drift, and disabled/offline W&B smoke pass after protocol repair 11 | `make ci-cpu`: 562 passed, 1 skipped in 146.42 seconds; 455.8 GB free before launch and 456.0 GB afterward; no GPU or online W&B work ran |
