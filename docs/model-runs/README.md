...
| --- | --- | --- | --- | --- | ---: | --- | --- |
| [VAL-001 held-out validation](VAL-001-held-out-validation.md) | VAL-001 | draft (stacked on DATA-004) | not exposed by runtime / not exposed by runtime | pending | 0 | in progress | evaluation-parity, token-weighting, data-separation, checkpoint-identity |
| [DATA-004 pinned baseline mixture](DATA-004-pinned-baseline-mixture.md) | DATA-004 | [#41](https://github.com/Ayumu-J-S/llm_scratch/pull/41) (ready for human review) | not exposed by runtime / not exposed by runtime | FAIL `4680931313`; model/mode not exposed | 2 | PASS WITH NOTE `4681064118`; no actionable technical finding remains; unresolved source-rights policy gate blocks agent self-merge | data-quality, split-leakage, document-identity, token-mixture, cache-headroom, source-provenance, iterator-lifecycle, evidence-completeness, config-reproducibility, legal-policy |
| [GATE-001 post-merge finalization](GATE-001-postmerge-finalize.md) | GATE-001 | [#40](https://github.com/Ayumu-J-S/llm_scratch/pull/40) (merged `7648316`) | not exposed by runtime / not exposed by runtime | FAIL `4680818667`; model/mode not exposed | 1 | PASS — final review `4680826547`; guarded audit and squash merge complete | post-merge-status, dependency-reconciliation, audit-parity, stale-final-status, provenance-target |
...
| --- | ---: | ---: | ---: | ---: | ---: | --- | --- | --- |
| not exposed by runtime / not exposed by runtime | 22 | 8 | 46 | 27 | 63 | Produced scoped EXP-001, DATA-001, DATA-002, MODEL-001, ENV-001, CFG-001, REP-001, LOOP-001, STAB-001, CKPT-001, CI-001, GEN-001, GATE-001, DATA-004, guarded-policy, canonical-tokenizer, provenance, roadmap-state maintenance, and documentation-audit changes; localized causal-boundary, immutable-manifest, tokenizer, model-invariant, CUDA-boundary, profile/preflight, source-separation, run-identity, global-seeding, training-loop, token-weighting, BF16 accumulation/clipping/schedule, bounded-stream shutdown, full-state recovery, rotation, offline-CI parity, checkpoint-owned deterministic generation, fixed-fixture end-to-end evidence, pinned-corpus streaming, trained-target mixtures, and dependency-state semantics | One implementation stalled; two review attempts blocked; previous review findings remain as recorded above. CKPT-001 initially conflated completed stream iteration with explicit resume, causing one full-suite failure before a cursor-pending repair; a CPU smoke then found duplicate epoch-end milestone saves before a persisted event-state repair; an exact-resume audit rejected cursorless map-style loaders rather than silently replaying a prefix. Formal review `4680194134` found terminal completed-cursor resume returned no next pass; formal re-review `4680225857` found recorded run Git/lock/experiment identity omitted from resume compatibility. The run-identity repair then passed independent re-review `4680237382` with a bounded-evidence note. CI-001 review retained a documented process-scope limitation for its socket guard. GEN-001 required a pre-review same-shape model-identity parity guard before passing independent review `4680305424`. GATE-001 initially needed fixture sizing, a non-terminal recovery point, unambiguous predeclared samples, and one formatter-only repair; its review retained an environment-scoped determinism note. DATA-004 live validation exposed an early-termination Parquet cache lease before formal review; an explicit iterator close and regression repaired it. DATA-004 review `4680931313` then found missing content-bound IDs, CHECK §4/R2/R3 evidence, quota-truncation accounting, and reproducible resolved configs. Repair cycle 2 addressed all four technical findings at `fee0f1a` with rerun evidence at `458dbdc`; independent re-review `4681064118` returned PASS WITH NOTE and counted the repair successful. The remaining source-rights disposition is a human policy gate, not a technical review failure. ROADMAP-MAINT passed its first independent R0 review with no findings. Exact model attribution remains impossible | 2026-07-12 |
