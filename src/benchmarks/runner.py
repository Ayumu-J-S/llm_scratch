...
_EVALUATION_DETERMINISM_POLICY = {
...
def _apply_evaluation_determinism_policy() -> dict[str, Any]:
    """Apply the fixed, fail-closed benchmark execution policy.

    The cuBLAS workspace choice must be present before CUDA initializes.  A
    caller that already initialized CUDA under another policy cannot safely be
    repaired in-process, so benchmark execution fails instead of publishing a
    result under an identity it did not execute.
    """

    required_workspace = str(_EVALUATION_DETERMINISM_POLICY["cublas_workspace_config"])
    ...
    if torch.cuda.is_initialized() and configured_workspace != required_workspace:
        raise RuntimeError(
            "benchmark determinism requires CUBLAS_WORKSPACE_CONFIG="
            f"{required_workspace} before CUDA initialization"
        )
    os.environ["CUBLAS_WORKSPACE_CONFIG"] = required_workspace

    seed_everything(int(_EVALUATION_DETERMINISM_POLICY["seed"]), deterministic=True)
    ...
    for key in ("deterministic_algorithms", "scaled_dot_product_attention", "cudnn", "float32"):
        if observed[key] != _EVALUATION_DETERMINISM_POLICY[key]:
            raise RuntimeError(
                f"benchmark determinism policy was not applied for {key}: {observed[key]}"
            )
    return dict(_EVALUATION_DETERMINISM_POLICY)
